  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Force hard reload so navbar catches the auth cookie immediately
  ...
  const handleGoogleLogin = async () => {
    try {
      await authClient.signIn.social({
        ...
        errorCallbackURL: "https://digitallife-client.vercel.app/register?error=oauth_failed",
      });
    } catch (err) {
      ...
      <Card className="w-full max-w-md p-8">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            ...
            {/* 2. Wrap input in relative div and add the toggle button */}
            <div className="relative">
              <input 
                type={showPassword ? "text" : "password"} 
                required
                className="w-full px-4 py-2 bg-[--bg-secondary] border border-[--border] rounded-md focus:outline-none focus:border-[--accent] text-[--text] pr-10"
